...
// submitパラメータの値が存在する時（「更新」ボタンを押した時）の処理
...
if (isset($_GET['id'])) {
    try {
        ...
        // idパラメータの値と同じidのデータが存在しない場合、エラーメッセージを表示して処理を終了
        if ($book === FALSE) {
            exit('idパラメータの値が不正です');
        }

        // genresテーブルからgenre_codeカラムのデータを取得するためのSQL文を変数$sql_select_genre_codesに代入
